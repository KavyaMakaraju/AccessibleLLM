...
stop_keyboard_flag = threading.Event()  # Use threading.Event for a more robust signaling mechanism

# Function to run the gaze-controlled keyboard
def run_keyboard():
    global text_data, stop_keyboard_flag
    ...
    while not stop_keyboard_flag.is_set():
        _, frame = cap.read()
        keyboard[:] = (26, 26, 26)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        # Your existing gaze-controlled keyboard logic here
        # Load sounds
        ...
        if cv2.waitKey(1) == 27:  # Check if the ESC key is pressed
            stop_keyboard_flag.set()  # Set the flag to True to stop the loop
...
def shutdown_server():
    ...
    uvicorn_server = app.state.uvicorn_server
    if uvicorn_server:
